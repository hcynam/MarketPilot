...
- `KAVENEGAR_VERIFY_TEMPLATE`: the exact approved Kavenegar Verify/Lookup template name used to deliver the Supabase OTP.
- `SUPABASE_SMS_HOOK_SECRET`: the server-only secret used to verify Supabase Send SMS Hook signatures.

Supabase generates and validates the OTP. The Netlify hook passes that exact OTP as the Kavenegar Verify/Lookup `token`; Kavenegar only delivers it.
