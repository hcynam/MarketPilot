...
- `KAVENEGAR_API_KEY`: the server-only Kavenegar API key used only to deliver the SMS generated by Supabase.
- `KAVENEGAR_SENDER` (optional): a real sender line enabled on the Kavenegar account. When omitted or empty, the hook does not send a `sender` parameter and Kavenegar may use the account default. Do not invent or hardcode this value.
- `SUPABASE_SMS_HOOK_SECRET`: the server-only secret used to verify Supabase Send SMS Hook signatures.

Supabase generates and validates the OTP. The Netlify hook sends Supabase's signed SMS payload through Kavenegar's ordinary `sms/send` endpoint; Kavenegar Verify/Lookup templates are not used.
